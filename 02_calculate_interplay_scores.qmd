...
def process_files(annotation_df, base_dir, input_dir, output_dir):
    ...
    for index, row in annotation_df.iterrows():
        ...
        try:
            ...
            # Pre-rename all columns (except for 'binary_comb' which is used for merging) to include unique suffix
            data_df.rename(columns={col: f'{col}_{unique_suffix}' for col in data_df.columns if col != 'binary_comb'}, inplace=True)
    ...
    for (condition, histone), df in condition_histone_dfs.items():
        ...
        interplay_columns = [col for col in df.columns if col.startswith('interplay_')]
        print(f"Interplay columns for averaging in {condition}_{histone}: {interplay_columns}")  # Debugging
        df['avg_interplay'] = df[interplay_columns].mean(axis=1)
        
        ac_interplay_columns = [col for col in df.columns if col.startswith('abundance_corrected_interplay_')]
        print(f"Abundance Corrected Interplay columns for averaging in {condition}_{histone}: {ac_interplay_columns}")  # Debugging
        df['avg_abundance_corrected_interplay'] = df[ac_interplay_columns].mean(axis=1)
        
        normalized_columns = [col for col in df.columns if col.startswith('normalized_interplay_')]
        print(f"Normalized Interplay columns for averaging in {condition}_{histone}: {normalized_columns}")  # Debugging
