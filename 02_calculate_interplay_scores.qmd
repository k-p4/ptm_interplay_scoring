...
```{python find and match PTMs from combos_1_interplay to combos_2_interplay then calculate I, ACI, NI}
import os
import re
import numpy as np
import pandas as pd
...
def process_combo_pair(combos_1_file, combos_2_file, output_dir):
    ...
    # Calculate normalized interplay score
    # df_combos_2['normalized_interplay'] = ((np.log(df_combos_2['binary_abundance'] / 
    #                                   (df_combos_2['discrete_abundance_1'] * df_combos_2['discrete_abundance_2'])) /
    #                                        (-np.log(df_combos_2['binary_abundance']))))
                                           
    df_combos_2['normalized_interplay'] = df_combos_2.apply(
        lambda row: -1 if row['binary_abundance'] == 0 and row['discrete_abundance_1'] > 0 and row['discrete_abundance_2'] > 0 else
        np.log(row['binary_abundance'] / (row['discrete_abundance_1'] * row['discrete_abundance_2'])) / (-np.log(row['binary_abundance'])),
        axis=1
    )
