...
# Step 2: Function to calculate new columns
def calculate_interplay_scores(df):
    # Calculate 'interplay'
    df['interplay'] = np.log(df['Percentage Abundance'] / (df['PTM1_abundance'] * df['PTM2_abundance']))

    # Calculate 'abundance_corrected_interplay'
    df['abundance_corrected_interplay'] = np.log((df['Percentage Abundance'] * (1 - df['PTM1_abundance']) * (1 - df['PTM2_abundance']) * df['Percentage Abundance']) /
                                                 (df['PTM1_abundance'] * df['PTM2_abundance'] * (df['PTM1_abundance'] - df['Percentage Abundance']) * (df['PTM2_abundance'] - df['Percentage Abundance'])))

    # Calculate 'normalized_interplay'
    df['normalized_interplay'] = df['interplay'] / (-np.log(df['Percentage Abundance']))

    # Handling special cases:
    # Case 1
    mask1 = (df['Percentage Abundance'] == 0) & (df['PTM1_abundance'] > 0) & (df['PTM2_abundance'] > 0)
    df.loc[mask1, ['interplay', 'abundance_corrected_interplay']] = np.nan
    df.loc[mask1, 'normalized_interplay'] = -1

    # Case 2
    mask2 = (df['Percentage Abundance'] == 1) & (df['PTM1_abundance'] == 1) & (df['PTM2_abundance'] == 1)
...
for file_path in glob.glob(file_pattern):
    ...
    df = calculate_interplay_scores(df)
